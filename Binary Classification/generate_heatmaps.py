...
def process_attention(attn_1d, image):
    ...
    heat = cv2.applyColorMap(np.uint8(weights * 255), cv2.COLORMAP_JET)
...
videos = sorted(
    [d for d in os.listdir(FRAME_ROOT) if os.path.isdir(os.path.join(FRAME_ROOT, d))]
)
...
for idx, video in enumerate(videos):
    ...
    mean_heat, mean_overlay = process_attention(mean_attention, image)
    ...
    for h in range(num_heads):
        ...
        heat, _ = process_attention(attn_heads[h], image)

        plt.subplot(3, cols, cols + h + 1)
    ...
    for h in range(num_heads):
        ...
        _, overlay = process_attention(attn_heads[h], image)

        plt.subplot(3, cols, 2 * cols + h + 1)
    ...
    save_path = os.path.join(save_dir, video + ".png")
